...
function Labels({ boxes, onDelete, onEdit, onSelect, imgWidth, imgHeight, user, folder, name, selectedFolder, setSelectedFolder, colors, hexToRgba }) {
  ...
  return (
    <div className="h-full bg-white flex flex-col h-full rounded-lg shadow-sm overflow-hidden">
      {boxes.length > 0 &&
      <div className="p-4 border-b border-gray-200 flex flex-row gap-2 overflow-auto">
        <div className="relative w-full">
          <FaSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" />
          <input
            type="text"
            placeholder="Search annotations..."
            className="w-30 sm:w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md bg-gray-50 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            value={searchTerm}
